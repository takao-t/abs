...
echo "endpoint/dtls_ca_file = /etc/asterisk/keys/abs.crt"
...
for i in 1 2
do
    echo "[doorphone$i](phone-defaults)"
    SALT=`date +%N`
    SECBASE="phone:$SALT:$i"
    SECRET=`echo $SECBASE | md5sum | cut -f1,1 -d' '`
    echo "inbound_auth/username = doorphone$i"
    echo "inbound_auth/password = $SECRET"
    echo "outbound_auth/username = doorphone$i"
    echo "outbound_auth/password = $SECRET"
    echo "endpoint/context = from-door"
    echo "endpoint/callerid = \"ドアホン$i\" <500$i>"
    echo "endpoint/allow=ulaw,h264"
    echo ""
done
