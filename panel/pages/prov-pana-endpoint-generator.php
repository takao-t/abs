...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['function']) && $_POST['function'] === 'createendpoint') {
    ...
    for ($i = 1; $i <= $max_phones; $i++) {
        if (isset($_POST["create_{$i}"]) && $_POST["create_{$i}"] === 'yes') {
            ...
            if ($type === 'fap') {
                $p_endpoint = $_POST["endpoint_{$i}"];
                $p_password = $passwords[$p_endpoint] ?? '';
                $content .= "## SIP Settings(LINE1)\n";
                $content .= "PHONE_NUMBER_1=\"{$p_endpoint}\"\n";
                $content .= "SIP_URI_1=\"{$p_endpoint}\"\n";
                $content .= "SIP_AUTHID_1=\"{$p_endpoint}\"\n";
...
if ($type === 'fap') {
        ...
        if ($mac_addr !== '') {
            foreach ($mac_pana_vc as $prefix) {
                ...
                if (stripos($mac_addr, $clean_prefix) === 0) {
                    $provisionable_phones[] = ['endpoint_num' => $i, 'endpoint_name' => $endpoint_name, 'mac' => $mac_addr];
                    break;
                }
            }
        }
    ...
    for ($i = 1; $i <= $max_sip_phones; $i++) {
        ...
        $mac_addr = $ami->getDbItem("ABS/PINFO/{$endpoint_name}", 'MAC');
        $exten = $ami->getDbItem("ABS/ERV/PJSIP", $endpoint_name);
        if ($mac_addr !== '' && $exten !== '') {
            foreach ($mac_pana_vc as $prefix) {
                $clean_prefix = str_replace(':', '', $prefix);
                if (stripos($mac_addr, $clean_prefix) === 0) {
                    $provisionable_phones[] = ['endpoint_num' => $i, 'endpoint_name' => $endpoint_name, 'mac' => $mac_addr, 'exten' => $exten];
                    break;
                }
            }
        }
...
?>
<h2>パナソニック <?= ($type === 'fap') ? 'フリーアドレス用' : '' ?>エンドポイント設定ファイル生成</h2>
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="ohans_<?= $phone['endpoint_num'] ?>">
                                <option value="yes" selected>する</option>
                                <option value="no">しない</option>
                            </select>
                        </td>
                        <td style="text-align: center;">
                            <input type="hidden" name="endpoint_<?= $phone['endpoint_num'] ?>" value="<?= htmlspecialchars($phone['endpoint_name'], ENT_QUOTES, 'UTF-8') ?>">
                            <input type="hidden" name="mac_<?= $phone['endpoint_num'] ?>" value="<?= htmlspecialchars($phone['mac'], ENT_QUOTES, 'UTF-8') ?>">
                            <?php if ($type === 'normal'): ?><input type="hidden" name="exten_<?= $phone['endpoint_num'] ?>" value="<?= htmlspecialchars($phone['exten'], ENT_QUOTES, 'UTF-8') ?>"><?php endif; ?>
                            <input type="checkbox" name="create_<?= $phone['endpoint_num'] ?>" value="yes">
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
