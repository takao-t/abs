...
EOT;
    ...
    header('Location: index.php?page=prov-gs-endpoint');
    exit;
}
...
for ($i = 1; $i <= $max_sip_phones; $i++) {
    $endpoint_name = "phone{$i}";
    $mac_addr = trim($ami->getDbItem("ABS/PINFO/{$endpoint_name}", 'MAC'));
    $exten = trim($ami->getDbItem("ABS/ERV/PJSIP", $endpoint_name));
...
?>
<h2>Grandstream エンドポイント設定ファイル生成</h2>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <input type="hidden" name="exten_<?= $phone['endpoint_num'] ?>" value="<?= htmlspecialchars($phone['exten'], ENT_QUOTES, 'UTF-8') ?>">
